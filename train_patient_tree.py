...
def run_patient_tree(X, y_patient, out_dir=".", feature_names=None, class_labels=None):
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    y = pd.Series(y_patient)
    ...
    cv_results.to_csv(out_dir / "cv_results_patient.csv", index=False)
    ...
    metrics_df.to_csv(out_dir / "metrics_patient.csv", index=False)
    ...
    ax.set_title("Confusion Matrix (Patient, CV aggregated)")
    fig.tight_layout()
    fig.savefig(out_dir / "fig_patient_confusion.png", dpi=150)
    plt.close(fig)
    ...
    ax.set_title("Top 15 Feature Importances (Patient)")
    fig.tight_layout()
    fig.savefig(out_dir / "fig_patient_importances.png", dpi=150)
    plt.close(fig)

    dot_path = out_dir / "tree_patient.dot"
    export_graphviz(
        final_model,
    ...
    try:
        subprocess.run(
            ["dot", "-Tpng", str(dot_path), "-o", str(out_dir / "tree_patient.png")],
            check=True,
        )
